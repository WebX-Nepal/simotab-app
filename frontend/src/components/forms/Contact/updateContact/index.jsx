...
const UpdateContact = () => {
  ...
  const handleContactUpdateSubmit=async(values)=>{
    const response=await updateDataWithHeader(`contacts/${id}`,values,token)

    console.log(response)

    if(response.success){
        successToast(response.message?response.message:"Contact updated successfully",token)
        navigate('/admin/contacts')
        
    }else{
        errorToast(response.message?response.message:"unable to update the contact")
    }
